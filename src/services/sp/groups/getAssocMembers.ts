...
export async function getAssociatedMembersGroup( webUrl: string, ): Promise<IGroupResults> {
  ...
  try {
    ...
    if ( check4This( 'fpsShowFetchResults=true' ) === true ) { console.log( `fps-Pnp2 SUCCESS: getAssociatedMembersGroup ~ 23`, group, group ) ; };

    return { group: group, e: null, status: 'Success' }

  } catch (e) {

    if ( check4This( 'fpsShowFetchResults=true' ) === true ) { console.log( `fps-Pnp2 ERROR: getAssociatedMembersGroup ~ 29`, e ) ; };
