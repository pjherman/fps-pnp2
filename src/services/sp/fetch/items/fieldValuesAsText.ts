...
export interface IMinFetchItemAsText extends IMinItemFetchProps {
  webUrl: string;
  listTitle: string;
  Id: number;
  selectThese?: string[];
 ...
 * @returns 
 */
export async function fetchItemAsText( fetchProps: IMinFetchItemAsText, ) : Promise<IItemErrorObj> {
  ...
  if ( !listTitle ) {
    ...
    try {
      ...
      if ( check4This( 'fpsShowFetchResults=true' ) === true ) { console.log( `fps-Pnp2 Success: fetchItemAsText ~ 55`, result ) };

    } catch (e) {

      if ( check4This( 'fpsShowFetchResults=true' ) === true ) { console.log( `fps-Pnp2 ERROR: fetchItemAsText ~ 59`, e ) };
